...
public class WebSecurityConfig  extends WebSecurityConfigurerAdapter {
    ...
    public void configure(WebSecurity web) { // static 하위 파일 목록(css, js, img) 인증 무시
        web.ignoring().antMatchers("/static/bootstrap/css/**", "/static/bootstrap/js/**", "/static/bootstrap/img/**", "/h2-console/**");
    }
    ...
    protected void configure(HttpSecurity http) throws Exception { // http 관련 인증 설정
        http
                ...
                .antMatchers("/login", "/signup", "/user","/","/bun=기흥").permitAll() // 누구나 접근 허용
//                .antMatchers("/after").hasRole("USER") // USER, ADMIN만 접근 가능
